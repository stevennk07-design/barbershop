  ...
  return (
    <div>
      <div className="space-y-2 mb-4">
        {services.map((s) => (
          <div key={s.id} className={`bg-neutral-950 border border-neutral-800 rounded-lg p-3 ${!s.active ? 'opacity-50' : ''}`}>
            <input
            ...
            <div className="flex items-end gap-2">
              ...
              <button onClick={() => toggleActive(s)} className="text-xs px-2 py-1.5 rounded border border-neutral-800 hover:bg-neutral-900 text-neutral-300">
                {s.active ? 'Active' : 'Hidden'}
              </button>
              <button onClick={() => deleteService(s.id)} className="text-red-400 hover:bg-red-950 px-2 py-1.5 rounded text-sm">×</button>
            </div>
          </div>
        ))}
      </div>
